...
function getElementRecursively(element) {
  return 
}
  ...
  if (process.client && query.token) {
    ...
    window.onmessage = ({ data }) => {
      if (data.focusedElement) {
        const identifier = getElementRecursively(data.focusedElement)
        const element = document.querySelector(`[data-${targetIdentifier}='${identifier}']`)

        log(data.focusedElement)
